...
static void print_bottom_set(void)
{
    ...
    uint8_t buf[4] = { 0 };
    uint8_to_asciiz((uint8_t)control_task_setpoints.bottom_heater_setpoint, (char *)buf);
    zprint((char *)buf, NORM);
}
#endif
...
void manual_heater_control(void)
{
    ...
    while (true) {
        ...
        if (key_code == UP_KEY_RELEASED) {
        ...
        } else if (((key_code == ENTER_KEY_RELEASED) || start_trigger) && (!lock)) {
            print_top_set(temp_setpoint);
            ...
            if (xTaskNotify(control_taskHandle, HEATER_START, eSetValueWithOverwrite) != pdPASS) {
                // errror
            }
            ...
            if (xTaskNotifyWait(ULONG_MAX, ULONG_MAX, &notif_val, pdMS_TO_TICKS(600U)) != pdTRUE) {
                /* error */
                /* stop */
                start_trigger = false;
                lock          = true;
            ...
            if (notif_val != CONTROL_TASK_RES_OK) {
                log_xprintf(MSG_LEVEL_SERIOUS, "Control task notif. = %d", notif_val);
                start_trigger = false;
                lock          = true;
            }
            gotoXY(20, 40);
            ...
            if ((HAL_GetTick() - elapsed) >= 1000U) {
                elapsed = HAL_GetTick();
                proc_time++;
                xprintf("%d, %d, %d, %d, %d\n", proc_time, temp_setpoint, temperature, throttle,
                        unclamped_result);
            }
        } else {
            //
...
void start_reflow(uint8_t np)
{
    ...
    control_task_setpoints.top_heater_setpoint    = 0U; //SETPOINT_UPPER_LIM_TOP;
    control_task_setpoints.bottom_heater_setpoint = 0U; //SETPOINT_UPPER_LIM_BTM;
    control_task_setpoints.mains_half_period      = get_mains_half_period();
    ...
    log_xprintf(MSG_LEVEL_EXT_INF, "Estimated time = %d seconds.", estimated_time);
    ...
    */
    ...
    uint16_t elapsed_step_time  = 0U; ///< with respect to the fastest permissible heat rate
    uint16_t temperature        = 0U; ///< actual temperature
...
#define START_TEMP (50U)
    ...
    while (true) {
        ...
        if (!pre_heat) {
            ...
            _Bool min_step_duration_expired = (elapsed_step_time >= curr_step->min_duration);
            _Bool temp_diff = !u16_eq_tol(temperature, curr_step->target_temperature, 2U);
            ...
            if (!extra_time_added && min_step_duration_expired && temp_diff) {
                /* we need more time */
                extra_time       = curr_step->max_duration - curr_step->min_duration;
                extra_time_added = true;
#if (0)
                xprintf("add extra time %d sec for step ", extra_time);
            ...
            if (!extra_time_added) {
                set_point_temp = profiles_get_temp_for_step_time(curr_step, elapsed_step_time);
            } else {
        ...
        if (prev_proc_time < total_process_time) {
            ...
            xprintf("%d, %d, %d, %d, %d, %d, %d\n", total_process_time, elapsed_step_time,
                    set_point_temp, temperature, throttle, extra_time, unclamped_result);
