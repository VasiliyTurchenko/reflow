 ...
 * @return
 */
uint16_t profiles_get_temp_for_step_time(const profile_point_t *running_profile_point,
                                         uint16_t               step_time)
{
    int32_t retVal = 0;
 ...
 * @return
 */
static inline const profile_point_t *get_current_step(const reflow_profile_t *running_profile,
                                                      uint16_t                time)
{
    const profile_point_t *curr_step      = running_profile->first;
    uint16_t               steps_end_time = curr_step->min_duration; ///< accumulated steps time
 ...
 * @return
 */
static inline uint16_t get_current_step_end_time(const reflow_profile_t *running_profile,
                                                 uint16_t                time)
{
    const profile_point_t *curr_step      = running_profile->first;
    uint16_t               steps_end_time = curr_step->min_duration; ///< accumulated steps time
 ...
 * @return
 */
uint16_t profiles_get_remaining_step_time(const reflow_profile_t *running_profile, uint16_t time)
{
    uint16_t retVal = 0U;
 ...
 * @return
 */
const char *profiles_get_step_name(const reflow_profile_t *running_profile, uint16_t time)
{
...
uint8_t profiles_get_temp_for_time(const reflow_profile_t *running_profile, uint16_t time)
{
    ...
    do {
        if ((time >= elapsed_sec) && (time < (elapsed_sec + curr_step->min_duration))) {
            /* this is the step */
