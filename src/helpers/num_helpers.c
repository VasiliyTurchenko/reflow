...
numeric_t str_to_num(const char *s)
{
    ...
    if (*s == 0x00U) {
        ...
        if (!val_signed) {
            ...
            need_float_result = true; /*the result doesn't fit into unsigned inegers */
        } else {
            /* signed result */
            if (decoded_it <= ABS_INT8_MIN_64B) {
                retVal.type       = S8_VAL;
                retVal.val.i8_val = (int8_t) - (int8_t)((uint8_t)decoded_it);
                goto fExit;
            }
            if (decoded_it <= ABS_INT16_MIN_64B) {
                retVal.type        = S16_VAL;
                retVal.val.i16_val = (int8_t) - (int16_t)((uint16_t)decoded_it);
                goto fExit;
            }
            if (decoded_it <= ABS_INT32_MIN_64B) {
                retVal.type        = S32_VAL;
                retVal.val.i32_val = -(int32_t)((uint32_t)decoded_it);
                goto fExit;
            }
            need_float_result = true; /*the result doesn't fit into unsigned inegers */
        }
