...
{
    SPI_HandleTypeDef       *OLED_hspi = (SPI_HandleTypeDef *)OLED_hspi_;
...
#define WriteCmdSSD1303 WriteCmdSSD1306
    ...
    WriteCmdSSD1303(OLED_hspi,
                    SET_COM_OUT_DIR_CMD | COM_REMAP); //Set COM Output Scan Direction
    ...
    WriteCmdSSD1303(OLED_hspi, 0x71 /*F0*/); //--set display clock divide ratio/oscillator frequency
    ...
    WriteCmdSSD1303(OLED_hspi,
                    SET_LOWER_NIBBLE_CA | (2 & 0x0F));      //---set low column address
    ...
    WriteCmdSSD1303(OLED_hspi,
                    START_LINE_ADDRESS_CMD | DISPLAY_START_LINE); //--set start line address
