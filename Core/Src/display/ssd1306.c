...
//	WriteCmdSSD1306(OLED_hspi, 0x10); //00,Horizontal Addressing Mode;01,Vertical Addressing Mode;10,Page Addressing Mode (RESET);11,Invalid
	...
	WriteCmdSSD1306(OLED_hspi, 0xB0); //Set Page Start Address for Page Addressing Mode,0-7
	...
	WriteCmdSSD1306(OLED_hspi, 0xA4); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	WriteCmdSSD1306(OLED_hspi, 0xD3); //-set display offset
	WriteCmdSSD1306(OLED_hspi, 0x00); //-not offset
	WriteCmdSSD1306(OLED_hspi, 0xD5); //--set display clock divide ratio/oscillator frequency
	...
	WriteCmdSSD1306(OLED_hspi, 0xDA); //--set com pins hardware configuration
...
#define		WriteCmdSSD1303		WriteCmdSSD1306
	...
	WriteCmdSSD1303(OLED_hspi, SET_VCOM_LVL); WriteCmdSSD1303(OLED_hspi, HIGH_VCOM); //0x20,0.77xVcc
	WriteCmdSSD1303(OLED_hspi, SET_PRE_CHARGE_PERIOD); WriteCmdSSD1303(OLED_hspi, 0x22); //--set pre-charge period	
	WriteCmdSSD1303(OLED_hspi, SET_REMAP_0_0); //--set segment re-map 132 -> 0

	WriteCmdSSD1303(OLED_hspi, SET_COM_OUT_DIR_CMD | COM_REMAP); //Set COM Output Scan Direction

	WriteCmdSSD1303(OLED_hspi, SET_COM_PIN_CFG); WriteCmdSSD1303(OLED_hspi, COM_PIN_CFG_ALT); //--set com pins hardware configuration
	WriteCmdSSD1303(OLED_hspi, SET_MUX_RATIO); WriteCmdSSD1303(OLED_hspi, MUX_RATIO); //--set multiplex ratio(1 to 64)	
	WriteCmdSSD1303(OLED_hspi, SET_DISP_CLK_DIV); WriteCmdSSD1303(OLED_hspi, 0x71 /*F0*/); //--set display clock divide ratio/oscillator frequency
	WriteCmdSSD1303(OLED_hspi, SET_CONTRAST_CR); WriteCmdSSD1303(OLED_hspi, CONTRAST>>1); //--set contrast control register	
	WriteCmdSSD1303(OLED_hspi, SET_DISP_OFFSET); WriteCmdSSD1303(OLED_hspi, 0x00); //-set display offset
	WriteCmdSSD1303(OLED_hspi, SET_DISPLAY_NORM); //--set normal display
	WriteCmdSSD1303(OLED_hspi, SET_DCDC); WriteCmdSSD1303(OLED_hspi, DCDC_ON); //--set DC-DC enable
...
//	WriteCmdSSD1303(OLED_hspi, HOR_SCROLL_ON);
	...
	WriteCmdSSD1303(OLED_hspi, SET_MEM_ADDR_MODE_CMD); WriteCmdSSD1303(OLED_hspi, ADDR_MODE_HOR);
	
	WriteCmdSSD1303(OLED_hspi, SET_COL_ADDR_CMD); WriteCmdSSD1303(OLED_hspi, 0); WriteCmdSSD1303(OLED_hspi, 127);
	
	WriteCmdSSD1303(OLED_hspi, SET_LOWER_NIBBLE_CA | (2 & 0x0F)); //---set low column address
	WriteCmdSSD1303(OLED_hspi, SET_HIGHER_NIBBLE_CA | (0)); //---set high column address
	WriteCmdSSD1303(OLED_hspi,2);
	WriteCmdSSD1303(OLED_hspi,0x10);
	WriteCmdSSD1303(OLED_hspi, SET_DISPLAY_RAM); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	WriteCmdSSD1303(OLED_hspi, START_LINE_ADDRESS_CMD | DISPLAY_START_LINE ); //--set start line address
