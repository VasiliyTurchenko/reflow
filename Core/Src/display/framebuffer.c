...
struct_Cursor		myCur;
struct_FrameBuffer	fb1;
Struct_CharGen		cg8x8; 	/// for the 8x8 char generator
struct_Context		OLEDcontext = {&fb1, &cg8x8, &myCur};	// the first device context

struct_Context*		context1 = &OLEDcontext;

unsigned char	 FrameBuffer1[(128/CHAR_BIT)*(64)];
...
// ----------------------- sift byte string bitwise left -----------------
uint8_t sbfl(uint8_t *field, uint32_t flen, uint32_t ns)	// shifts bit field "* field" of "flen" bytes left by "ns" times
{
	...
	while (ns > 0) {
		...
		for (i = 0; i < flen; ++i) {
			...
			*(field+i) = (*(field+i) << 1) | carry_flag;
			carry_flag = next;
		}
	ns--;
	};
	return sns;						// return the actual nr of shifts
} // -------------------end of sift byte string bitwise left -----------------

// ----------------------- sift byte string bitwise right -----------------
uint8_t sbfr(uint8_t *field, uint32_t flen, uint32_t ns)	// shifts bit field "* field" of "flen" bytes right by "ns" times
{
	ns = (ns > flen*CHAR_BIT) ? flen*CHAR_BIT : ns;		// limit number of shifts
	uint32_t	sns;
	sns = ns;
	...
	next = 0;
		for (i = flen; i > 0; --i) {
			...
			*(field+i-1) = (*(field+i-1) >> 1) | carry_flag;
			carry_flag = next;
		}
	ns--;
	};
...
// -------------------end of sift byte string bitwise right -----------------
...
// --------------------end of fill byte string bitwise from left to right ------------
...
uint8_t fbfr(uint8_t *field, uint32_t flen, uint32_t ns)
{
	...
	carry_flag = 1;
	next = 0;
		for (i = flen - 1; i >= flen; --i) {
			next = (*(field+i) & 0x01) ? 0x80 : 0x00;
			*(field+i) = (*(field+i) >> 1) | carry_flag;
			carry_flag = next;
		}
	ns--;
	...
	return sns;						// return the actual nr of shifts
} // -------------------end of fill byte string bitwise from right to left-----------------

uint8_t	PutChar(char sym, uint8_t mode) 	// Puts the char and moves cursor to the next pos
{
	// check is char out of char gen table
	if ((sym < context1->pDevCHG->StartCharCode) || (sym > context1->pDevCHG->EndCharCode)) return 1; // no such symbol
	if ((context1->pDevFB->FBy0+context1->pDevCursor->CursorPosY) >= context1->pDevFB->FBHeight) return 2; // cursor is out of the framebuffer y limit
	if (context1->pDevCursor->CursorPosX >= context1->pDevFB->FBWidth) return 3; //  cursor is out of the framebuffer x limit
...
// 1. Copy symbol to temporary location
	...
	if ((context1->pDevCHG->CharWidth)%CHAR_BIT > 0) xcount++;	//	there is one extra NON-FULL byte in the row
        if (xcount > (MAX_CHAR_SIZE/CHAR_BIT)) xcount = (MAX_CHAR_SIZE/CHAR_BIT);
	ycount = context1->pDevCHG->CharHeight;
        if (ycount > (MAX_CHAR_SIZE)) ycount = (MAX_CHAR_SIZE);
	...
	for	(j = 0; j < ycount ; j++) {
		...
		for	(i = 0; i < xcount ; i++) {
			tmp_field[j][i] = sinv ^ *((context1->pDevCHG->pROM)+sym*context1->pDevCHG->BytesPerChar+i+j);
                    }
	}
	if (bitshifts > 0) {
	...
	poc = (context1->pDevFB->FBy0+context1->pDevCursor->CursorPosY)*\
				context1->pDevFB->FBWidth+context1->pDevCursor->CursorPosX; // offset to cursor in pixels
...
uint32_t zprint(char *sz, uint8_t mode)
{
	...
	while (*(sz+i) != 0) {
		if (*(sz+i) == 0x0A) {
			newx = 0;
			newy = newy + dy;
			if (gotoXY(newx, newy) == 0) {
				cnt++;
			} else break;
		} else {
//			if (PutChar(*(sz+i),mode) == 0 ) {
			if (PutChar1(*(sz+i),mode) == 0 ) {
				cnt++;
...
#define			ENDY		(context1->pDevCursor->CursorPosY + context1->pDevFB->FBy0 + context1->pDevCHG->CharHeight)
	uint32_t	xcount;
	uint32_t	ycount;
	xcount = (context1->pDevCHG->CharWidth)/CHAR_BIT;		// nr of full bytes
	if ((context1->pDevCHG->CharWidth)%CHAR_BIT > 0) xcount++;	//	there is one extra NON-FULL byte in the row
	...
	for (yc = STARTY; yc < ENDY; yc++){
		for	(i = 0; i < xcount ; i++) {
			tmp_field[i] = sinv ^ *((context1->pDevCHG->pROM)+sym*context1->pDevCHG->BytesPerChar+i+j);
			if (j == ycount - 1) tmp_field[i] |= underl;
		    }
		for (xc = STARTX; xc < ENDX; xc++ ) {
			zpointY(xc, yc, ((tmp_field[0] & 0x01) ? 1 : 0));
			sbfr(&tmp_field[0], TFS, 1);	// shifts bit field "* field" of "flen" bytes right by "ns" times
		}
		j++;
	}
#undef			STARTX
