...
uint8_t PutChar(char sym, uint8_t mode)
{
	...
	if ((sym < context1->pDevCHG->StartCharCode) ||
	    (sym > context1->pDevCHG->EndCharCode))
		return 1; // no such symbol
	if ((context1->pDevFB->FBy0 + context1->pDevCursor->CursorPosY) >=
	    context1->pDevFB->FBHeight)
	...
	uint32_t bitshifts;
	bitshifts = (context1->pDevCursor->CursorPosX) %
		    CHAR_BIT; // amount of the right shifts
	uint8_t sinv;	 // invert
	...
	for (j = 0; j < ycount; j++) {
		...
		for (i = 0; i < xcount; i++) {
			tmp_field[j][i] =
				sinv ^ *((context1->pDevCHG->pROM) +
					 sym * context1->pDevCHG->BytesPerChar +
					 i + j);
		}
	}
	if (bitshifts > 0) {
	...
	uint32_t incY, incX;
	incY = context1->pDevFB
		       ->FBWidthInBytes; // Y distance between points with same X coordinate
	...
	uint8_t tb;
	for (j = 0; j < ycount; j++) {
		if ((j + context1->pDevCursor->CursorPosY) <
		    context1->pDevFB->FBHeight) {
			for (i = 0; i <= xcount; i++) {
				if ((i + bocl) <
				    context1->pDevFB->FBWidthInBytes) {
					tb = *((context1->pDevFB->pFB) +
					       i * incX + j * incY + boc);
					tb = tb & ~mask_field[i];
					tb = (tmp_field[j][i] & mask_field[i]) |
					     tb;
					*((context1->pDevFB->pFB) + i * incX +
					  j * incY + boc) = tb;
				}
			}
		} else
...
//	uint32_t poc; // pixel_offset_to_dest point
	...
	res = ((context1->pDevFB->FBy0 + y) >= context1->pDevFB->FBHeight) ? 1 :
									     0;
	res = (context1->pDevFB->FBOrientatonX == false) ? 1 : 0;

	if (!res) {
		uint32_t poc; // pixel_offset_to_dest point
		poc = (context1->pDevFB->FBy0 + y) * context1->pDevFB->FBWidth +
		      x;      //
		uint32_t boc; // byte_offset_to_dest point
		boc = (poc * context1->pDevFB->BitsPerPixel) /
		      CHAR_BIT; // from 0
...
uint32_t zpointY(uint32_t x, uint32_t y, uint32_t color)
{
	...
	if (!res) {
		uint32_t boc; // byte_offset_to_dest point
		boc = ((context1->pDevFB->FBy0 + y) / CHAR_BIT) *
			      context1->pDevFB->FBWidth +
		      x;
		uint32_t pib; // position in the byte
...
#define ENDY                                                                   \
	(context1->pDevCursor->CursorPosY + context1->pDevFB->FBy0 +           \
	 context1->pDevCHG->CharHeight)
	uint32_t xcount;
	uint32_t ycount;
	xcount = (context1->pDevCHG->CharWidth) / CHAR_BIT; // nr of full bytes
	if ((context1->pDevCHG->CharWidth) % CHAR_BIT > 0)
